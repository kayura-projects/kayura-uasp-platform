...
public class MbMapperAnnotationBuilder extends MapperAnnotationBuilder {
  ...
  public MbMapperAnnotationBuilder(MbConfiguration configuration, Class<?> type) {
    ...
    if (!configuration.isResourceLoaded(resource)) {
      ...
      for (Method method : methods) {
        try {
          ...
          configuration.addIncompleteMethod(new MbMethodResolver(this, method));
        }
      }

      // 注入动态SQL
      if (configuration.isSupperMapperClass(type)) {
        configuration.inspectInject(assistant, type);
      }
    }
    parsePendingMethods();
  }
  ...
  /**
   * 解析一个方法上的注解，并向配置中心注册一个 Statement 方法。
   */
