...
public final class AnnotationUtils {
   ...
   * 组合注解属性值提取器缓存
   */
  private static final Map<SynthesizedAnnotationCacheKey, AttributeValueExtractor> synthesizedCache = new ConcurrentHashMap<>(
    256);
   ...
   * @return
   */
  public static <A extends Annotation> boolean isAnnotationPresent(AnnotatedElement annotatedElement,
                                                                   Class<A> annotationType) {
  ...
  public static Object getAttributeValue(Annotation annotation, String attributeName) {
    ...
    try {
      return ReflectUtils.invokeMethod(annotation.annotationType().getDeclaredMethod(attributeName), annotation);
    } catch (Exception ex) {
  ...
  public static Map<String, Object> getAttributeMap(Annotation annotation) {
    ...
    getAttributeMethods(annotation.getClass()).forEach(method -> {
      attributeMap.put(method.getName(), ReflectUtils.invokeMethod(method, annotation));
    });
    return attributeMap;
  }
   ...
   * @return
   */
  private static <A extends Annotation> A findMetaAnnotation(Annotation annotation, Class<A> targetType,
  ...
  private static <A extends Annotation> A synthesizeAnnotation(AnnotatedElement annotatedElement, Annotation annotation,
    ...
    return (A) Proxy.newProxyInstance(annotation.getClass().getClassLoader(),
      new Class<?>[]{annotation.annotationType()}, new SynthesizedAnnotationInvocationHandler(valueExtractor));
  }
  ...
  private static class DefaultAttributeExtractor implements AttributeValueExtractor {
     ...
     * @return 默认属性值提取器
     */
    private static DefaultAttributeExtractor from(AnnotatedElement element, Annotation target,
                                                  List<Annotation> visited) {
        ...
        for (OverrideDescriptor descriptor : overrideDescriptors) {
          if (descriptor.overrideAnnotationType == targetType) {
            ...
            if (!attributes.containsKey(targetAttributeName)) {
              attributes.put(targetAttributeName, ReflectUtils.invokeMethod(descriptor.sourceAttribute,
      ...
      for (Method targetAttribute : getAttributeMethods(targetType)) {
        ...
        if (!attributes.containsKey(attributeName)) {
          attributes.put(attributeName, ReflectUtils.invokeMethod(targetAttribute, target));
        }
      }
  ...
  public static class OverrideDescriptor {
    ...
    private OverrideDescriptor(Method sourceAttribute, OverrideFor overrideFor) {
      ...
      if (this.overrideAnnotationType == this.sourceAnnotationType) {
        String msg = String.format("@%s注解的%s属性方法上的@OverrideFor注解中申明的覆盖注解类型不能与该方法声明类一致!", declaringClass.getName(),
      ...
      try {
        ...
        String msg = String.format("@%s注解中%s属性方法上@OverrideFor注解对应的%s别名属性不存在于%s别名注解类中！",
      ...
      if (sourceAttribute.getReturnType() != overrideAttribute.getReturnType()) {
        String msg = String.format("'@%s'注解的'%s'属性方法类型与该方法上@OverrideFor所声明的'@%s'注解类型中的'%s'属性方法类型不匹配!",
          sourceAnnotationType.getName(), sourceAttributeName, overrideAnnotationType.getName(),
     ...
     * @return 如果 {@link OverrideFor}
     * 注解的attribute和value属性值都为空，则反回sourceAttribute的方法名
     */
  ...
  static public class SynthesizedAnnotationInvocationHandler implements InvocationHandler {
    ...
    public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
      ...
      if (ReflectUtils.isHashCodeMethod(method)) {
    ...
    private boolean annotationEquals(Object other) {
      ...
      for (Method attributeMethod : AnnotationUtils.getAttributeMethods(annotationType())) {
        ...
        Object otherValue = ReflectUtils.invokeMethod(attributeMethod, other);
